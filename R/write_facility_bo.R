...
#' 
#' @description This function is a `write_reports` function. It will generate summary report 
#'   for one specified facility. The summary workbook shows percents and counts of nulls and 
#'   invalids, Additionally it generates a timeliness. report and creates a table. The 
#'   program can send out a report to designated email address
#'
#' @param username Your BioSense username, as a string. This is the same username you may 
#'   use to log into RStudio or Adminer.
#' @param password Your BioSense password, as a string. This is the same password you may 
#'   use to log into RStudio or Adminer.
#' @param table The table that you want to retrieve the data from, as a string.
#' @param mft The MFT (master facilities table) from where the facility name will be 
...
#' @param facility The C_Biosense_Facility_ID for the facility that you wish to generate and 
#'   write the report for.
#' @param directory The directory where you would like to write the reports to 
#'   (i.e., "~/Documents/MyReports"), as a string.
#' @param field Default NA. Can add a string with delimiter of ':'. Only fields that contain 
#'   those words will be included in the final report.
#' @param exclude Default NA. Can add a string with delimiter of ':'. Exclude fields with 
#'   certain keywords in the final report.
...
#'
#' @return  A report table stored at directory location. If email=TRUE, then a email will 
#'   be sent, along with a confirmation of email being sent. 
#' 
#' @import dplyr
#' @import tidyr
#' @import openxlsx
...
write_facility_report <- function(username, password, table, mft, start, end, facility, directory="",
                                  ...
                                  email_password=NA, personname=NA, title=NA, phone=NA, message=NA) {
